...
- [Development guidelines](#development-guidelines)
    - [General Satcoin Core](#general-satcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Run with the `-testnet` option to run with "play satcoins" on the test network, if you
...
([`valgrind.supp`](https://github.com/minblock/satcoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_satcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_satcoin --log_level=test_suite
$ valgrind -v --leak-check=full src/satcoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_satcoin.coverage/index.html`.
```
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:minblock/satcoin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `satcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `satcoin-cli` and the GUI debug console use this table to determine how to
