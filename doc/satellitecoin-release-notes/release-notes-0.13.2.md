Satellitecoin Core version 0.13.2 is now available from:

  <https://download.provgn.com/satellitecoin-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a satellitecoin
...
ECDSA signatures inside Satellitecoin transactions now use validation using
...
Previous versions of Satellitecoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Satellitecoin Core was to raise the default minimum
relay fee.

Satellitecoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Satellitecoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running satellitecoind without having to do any manual configuration.
...
Satellitecoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Satellitecoin Core will relay transactions with
...
entirely in Satellitecoin Core 0.13, and it is at this time undecided whether the
...
Satellitecoin Core automatically creates a hidden service to listen on, without
manual configuration. Satellitecoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Satellitecoin Core is listening, and
...
Satellitecoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Satellitecoin
Core to use floating fees. By default, Satellitecoin Core will use floating
fees.
...
At all times, Satellitecoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) LTC.
Furthermore, Satellitecoin Core will never create transactions paying less than
...
`satellitecoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 222330 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 222330 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /satellitecoinrpc>
        ProxyPass http://127.0.0.1:22330/
        ProxyPassReverse http://127.0.0.1:22330/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass satellitecoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< satellitecoinrpc:<password>
    </Location>
...
Restarting satellitecoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in satellitecoin.conf
...
satellitecoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/satellitecoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Satellitecoin Core code base has
...
- `satellitecoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `satellitecoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Satellitecoin scripting system that in combination with
...
Satellitecoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Satellitecoin Core.
...
The two phases are distinct in the Satellitecoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Satellitecoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Satellitecoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Satellitecoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of satellitecoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of satellitecoins being spent is exactly
...
- **Increased security for multisig:** Satellitecoin addresses (both P2PKH addresses
  ...
  believed to be provided by Satellitecoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Satellitecoin's current ledger (called the UTXO set),
  ...
  note that Satellitecoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Satellitecoin users to individually opt-in to almost any change in the Satellitecoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Satellitecoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Satellitecoin
client to Satellitecoin Core 0.13.2 to signal for segwit support. When a
...
Since Satellitecoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `satellitecoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `satellitecoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
