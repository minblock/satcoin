Satcoin Core version 0.13.2 is now available from:
  ...
  <https://download.satcoin.org/satcoin-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a satcoin
...
This allows running satcoind without having to do any manual configuration.
...
`satcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /satcoinrpc>
        ...
        # optional bypass satcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< satcoinrpc:<password>
    </Location>
...
Restarting satcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in satcoin.conf
...
For example:
    ...
    $ src/satcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
- `satcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `satcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of satcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of satcoins being spent is exactly
...
- `satcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `satcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
