Satcoin Core version 0.13.2 is now available from:
...
ECDSA signatures inside Satcoin transactions now use validation using
...
Previous versions of Satcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Satcoin Core was to raise the default minimum
relay fee.

Satcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Satcoin Core 0.13.2 also introduces new default policy limits on the length and
...
Satcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Satcoin Core will relay transactions with
...
entirely in Satcoin Core 0.13, and it is at this time undecided whether the
...
Satcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Satcoin Core automatically creates a hidden service to listen on, without
manual configuration. Satcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Satcoin Core is listening, and
...
Satcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Satcoin
Core to use floating fees. By default, Satcoin Core will use floating
fees.
...
At all times, Satcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) STC.
Furthermore, Satcoin Core will never create transactions paying less than
...
Various code modernizations have been done. The Satcoin Core code base has
...
for a new opcode in the Satcoin scripting system that in combination with
...
Satcoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Satcoin Core.
...
The two phases are distinct in the Satcoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Satcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Satcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Satcoin block need to be stored by nodes in order to validate future
...
- **Increased security for multisig:** Satcoin addresses (both P2PKH addresses
  ...
  believed to be provided by Satcoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Satcoin's current ledger (called the UTXO set),
  ...
  note that Satcoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Satcoin users to individually opt-in to almost any change in the Satcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Satcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Satcoin
client to Satcoin Core 0.13.2 to signal for segwit support. When a
...
Since Satcoin Core 0.10.0, nodes have defaulted to only relaying and
